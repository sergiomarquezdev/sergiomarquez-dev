...
describe("getLatestCommit", () => {
  ...
  it("shortens SHA to 7 characters", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([createPushEvent({ sha: fullSha })]),
    );
  ...
  it("keeps short messages as-is without ellipsis", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([createPushEvent({ message: shortMsg })]),
    );
  ...
  it("truncates messages longer than 50 characters and adds ellipsis", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([createPushEvent({ message: longMsg })]),
    );
  ...
  it("uses only the first line of multi-line messages", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([createPushEvent({ message: multiLineMsg })]),
    );
  ...
  it("extracts repo name after the slash", async () => {
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([
        createPushEvent({ repoName: "sergiomarquezdev/portfolio-site" }),
      ]),
    );
  ...
  it("returns the created_at date from the event", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([createPushEvent({ createdAt: date })]),
    );
  ...
  it("keeps exactly 50-char messages without ellipsis", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([createPushEvent({ message: exactMsg })]),
    );
